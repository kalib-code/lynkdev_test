...
class Oxygen_VSB_Dynamic_List extends CT_Component {
    ...
    function add_shortcode( $atts, $content, $name ) {
        ...
        ?><div id="<?php echo esc_attr($options['selector']); ?>" <?php if( !empty( $gutenberg_placeholder ) ) echo 'gutenberg-placeholder="' . $gutenberg_placeholder . '" '; ?> class="<?php if(isset($options['classes'])) echo esc_attr($options['classes']); ?>"  <?php do_action("oxygen_vsb_component_attr", $options, $this->options['tag']); ?>><?php
		        ...
		        if(class_exists('ACF') && isset($options['use_acf_repeater']) && $options['use_acf_repeater'] !== 'false') {
			        ...
			        if(isset($options['acf_repeater']) && !empty($options['acf_repeater'])) {
                        ...
                        if(isset($oxygen_vsb_css_caching_active) && $oxygen_vsb_css_caching_active === true) {
                            if(!have_rows($options['acf_repeater'])) {
                                do_shortcode( $content );
                            }
                        }
                        
				        while ( have_rows($options['acf_repeater']) ) : the_row();
    ...
    function parse_shortcodes_map($models, $options, $parentQuery = false, $acfRepeaterFields = false) {

        $results = $this->parse_map_recursively($models, $options, $parentQuery, $acfRepeaterFields);
    ...
    function parse_map_recursively($models, $options, $parentQuery = false, $acfRepeaterFields = false) {
        ...
        if (isset($options['use_acf_repeater']) && $options['use_acf_repeater'] !== "false" && class_exists('ACF')) {
            ...
            if($acfRepeaterFields && is_array($acfRepeaterFields)) {

                foreach($acfRepeaterFields as $acfRepeaterField) {
    ...
    function resolve_recursively($arr) {
        ...
        if (is_array($arr)) {
            foreach($arr as $key => $value) {
                ...
                if ($arr['name'] === 'oxy_dynamic_list' && $key == 'children') {
                    $debug = true;
                    ...
                    <div class='oxygen-control-wrapper'>
                        ...
                        <div class='oxygen-control'>
                            ...
                            <div class='oxygen-four-sides-measure-box'>
                                ...
                                <?php $oxygen_toolbar->measure_box('paginatelinkactive_margin_top','',true,true,"model,change,keypress"); ?>
